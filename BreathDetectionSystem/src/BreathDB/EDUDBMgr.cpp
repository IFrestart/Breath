...
int CEDUDBMgr::SetSleepTime(TimeXlh &stru,char *cxlh)
{
	   ...
	   m_pDBTrace->TraceError("getSETime error : %d \n",ret);
        ...
        int flagtime = 0;   //鏃堕棿鏍囪
        int befortime = 0;  //鍓嶄竴涓椂闂?   
        int allSleepCount= 0;   //鎬荤潯鐪犺鏁?   
        int deepSleepCount = 0;
        int bedCount= 0;        //鍦ㄥ簥璁℃暟
        int awakCount= 0;       //瑙夐啋璁℃暟
        int leaveCount = 0;     //鏃犱汉璁℃暟
        int awakleaveCount = 0; //鍏ョ潯鍓嶆棤浜?   
        int NACount=0;         //杩炵画瑙夐啋璁℃暟
        int ahis = 0;
        int ahio = 0;
        WORD statflag = 0;   //鐘舵€佹爣璇?
...
int CEDUDBMgr::UpdateAbnormal(AbnormalStat &Astru)
{
    ...
    int flagtime = 0;   //时间标记
    int befortime = 0;  //前一个时间
    int allSleepCount= 0;   //总睡眠计数
    int deepSleepCount = 0;
    int bedCount= 0;        //在床计数
    int awakCount= 0;       //觉醒计数
    int leaveCount = 0;     //无人计数
    int NACount=0;         //连续觉醒计数
    int ahis = 0;
    int ahio = 0;
    WORD statflag = 0;   //状态标识
    for(int j=0; j<sAsize; j++)
    {
        flagtime = slArray[j].dwTime;
        statflag = slArray[j].wSleepStat1;

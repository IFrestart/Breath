	...
	USHORT nProxyPort, LPCTSTR pProxyUserName, LPCTSTR pProxyPassword)
{
	...
	CACSocket::GetSockName(m_pSocket->GetSocket(), szLocalHost, 
		&nLocalPort, szLocalMask);
	//m_pDataCallBack->OnNewConnection(m_pSocket->GetSocket(), 
		//szLocalHost, nLocalPort, szLocalMask, lpRemoteHost, nRemotePort);
	m_pDataCallBack->OnNewConnection(m_pSocket->GetSocket());
...
//modify by zjj 2009-5-25
			...
			else
			{
				...
				m_pHeadProc->GeneratePacketHeader(lpDataSend,wPduType, dwSize);
			}
//end modify by zjj 2009-5-25
		...
		else //使用HTTP协议
		{
			...
			m_pHeadProc->GeneratePacketHeader(&lpDataSend[dwPosIndex],wPduType, dwSize);
...
#else
	printf("CACTCPClientProc::StopMainProc pid=[%d]",pthread_self());
	void *status;
	pthread_mutex_lock(&m_mutexStopFlag);
	m_nStopFlag += 1;
	pthread_mutex_unlock(&m_mutexStopFlag);
...
printf("hhh  hSocket = [%d] pid=[%d]\n",hSocket,pthread_self());
	fd_set ReadFDs;
	int nPkgHeadSize = 0;
	WORD wExtParam = 0;
	WORD wPduType = 0;
...
//add by zjj 2012-10-12
		if (INVALID_SOCKET == m_pSocket->GetSocket())
		{
			break;
		}
//end add by zjj 2012-10-12
		...
		if(m_bHeadFlag)
		{
			...
			//m_dwDataSize = m_pHeadProc->GetAndParsePkgHead(m_pPackHead, 
				//&wPduType, &wExtParam);
			nRet = m_pHeadProc->ParsePacketHeader(m_pPackHead, wPduType, m_dwDataSize);
			...
			if (m_dwDataSize == 0)
			{
				...
				m_pDataCallBack->OnTCPPacketReceived(m_pSocket->GetSocket(),wPduType, NULL, 0);
	...
	//printf(" ACTCPClientProc.cpp after ioctl m_dwDataSize[%d] \n",m_dwDataSize);
			...
			while(1)
			{
				...
				if (m_dwDataSize == m_dwRDataBytes)
				{
					...
					m_pDataCallBack->OnTCPPacketReceived(m_pSocket->GetSocket(), wPduType,m_pPackData, m_dwDataSize);

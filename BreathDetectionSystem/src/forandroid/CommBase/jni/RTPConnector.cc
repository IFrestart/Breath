...
void* CRTPConnectorTpl<SOCKHEADER>::threadProc(void* arg)
{
	...
	_INFO("RTPConnector::threadProc() start thread %d", pthread_self());
...
//end add by zjj 2006-8-19
	...
	_ERROR("CRTPConnectorTpl<SOCKHEADER>::threadProc(=%d) thread exit!!!", pthread_self());

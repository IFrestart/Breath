...
int CACUDPSession::InitializeSession(ISockEventCallBack* lpDataCallBack,
	ISockHeaderCallBack* lpHeadProc, LPCTSTR lpLocalHost, USHORT nLocalPort)
{
...
int CACUDPSession::InitializeSession(ISockEventCallBack* lpDataCallBack, ISockHeaderCallBack* lpHeadProc, LPCTSTR lpLocalHost,
...
int CACUDPSession::SessionMainLoop(void)
{
	...
	while (1)
	{
		...
		if(nResult > 0)
		{
			...
			nResult = m_pHeadProc->ParsePacketHeader(dataRecv, wPduType, nDataSize);
			...
			if (nDataSize+nPkgHeadSize <= nSize)
			{
				...
				m_pDataCallBack->OnUDPPacketReceived(pACSocket->GetSocket(), wPduType,
					(char*)dataRecv + nPkgHeadSize, nSize - nPkgHeadSize, ip_to_long(szRemoteHost), nRemotePort);
			}
		}
	}	

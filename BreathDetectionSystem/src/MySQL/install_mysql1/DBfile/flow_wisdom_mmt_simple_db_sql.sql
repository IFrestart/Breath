...
create table phoneInfo(
		...
		day_flow int unsigned not null,
		used_day_flow int unsigned not null default 0,
		month_flow int unsigned not null,
		used_month_flow int unsigned not null default 0,
		pass_flow  int unsigned not null default 80,
		isCut tinyint(1) not null default 1,
		isCall tinyint(1) not null default 1,
		call_interval int unsigned not null default 5,
...
CREATE EVENT UPDATE_UsedMonthFlow_EVENT 
...
DO 
update phoneInfo set used_month_flow=0; 


CREATE EVENT UPDATE_UsedDayFlow_EVENT  
ON SCHEDULE EVERY 1 DAY STARTS DATE_ADD(DATE_ADD(DATE_SUB(CURDATE(),INTERVAL HOUR(CURDATE())-0 HOUR), INTERVAL 1 DAY),INTERVAL 0 HOUR) 
ON COMPLETION PRESERVE ENABLE 
DO  
update phoneInfo set used_day_flow=0; 

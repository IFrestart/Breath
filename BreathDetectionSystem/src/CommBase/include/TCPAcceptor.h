...
class CTCPAcceptorTpl : public ISvcManager, public ACE_Acceptor<SVCHANDLER, ACE_SOCK_ACCEPTOR>
{
public:
	CTCPAcceptorTpl(ISockEventCallBack* pSockEventCallBack = 0);
	/**
...
private:
	...
	{
	public:
		TimerHandler(CTCPAcceptorTpl<SVCHANDLER>*);
		int start();
		int kill();
	...
	private:
		...
		enum {CHECK_TIMER_ID = 1, CHECK_TIMEOUT_INTERVAL = 5000, CLIENT_TIMEOUT_INTERVAL = 1800}; //modify 2014-4-24
	//	enum {CHECK_TIMER_ID = 1, CHECK_TIMEOUT_INTERVAL = 5000, CLIENT_TIMEOUT_INTERVAL = 120}; //modify 2014-4-24
		CTimer		checkTimer_;
		CTCPAcceptorTpl<SVCHANDLER>*	pAcceptor;
	};
	...
	enum {SVC_HANDLER_NUM = 10000}; //modify 2013-5-23 by zjj
...
#include "../TCPAcceptor.cc"
typedef CTCPAcceptorTpl<CTCPSvcHandler>	CTCPAcceptor;

typedef CTCPAcceptorTpl<CTCPRECSvcHandler> CTCPRECAcceptor;
